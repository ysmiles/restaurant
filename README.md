...
To use [Nginx](https://nginx.org) serve this React app, first run building process

```shell
npm run build
```

Then copy the content in `build` to the root of Nginx served.
...
In React codes we can let browser call `fetch` to other back-end API.
